...
-- Version: 1.0.0 (Fixed for new role system)
...
CREATE TYPE user_role AS ENUM ('admin', 'operator', 'worker', 'viewer');
...
CREATE POLICY "Admin and operators can manage mills" ON mills FOR ALL TO authenticated USING (
...
CREATE POLICY "Admin and operators can manage separators" ON separators FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator'))
);

-- Products policies
CREATE POLICY "All authenticated users can view products" ON products FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admin and operators can manage products" ON products FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator'))
);

-- Silos policies
CREATE POLICY "All authenticated users can view silos" ON silos FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admin and operators can manage silos" ON silos FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator'))
);

-- Silo product rules policies
...
CREATE POLICY "Admin and operators can manage silo_product_rules" ON silo_product_rules FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator'))
);
...
CREATE POLICY "Operators can insert production sessions" ON production_sessions FOR INSERT TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator', 'worker'))
);
CREATE POLICY "Operators can update production sessions" ON production_sessions FOR UPDATE TO authenticated USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator', 'worker'))
);
...
CREATE POLICY "Operators can insert silo logs" ON silo_level_logs FOR INSERT TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator', 'worker'))
);
...
CREATE POLICY "Workers can insert packaging entries" ON packaging_entries FOR INSERT TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('admin', 'operator', 'worker'))
);
